...
def transform_name(product_name, stemmer=True):
    ...
    # string.puctuation is !"#$%&'()*+,-./:;<=>?@[\]^_`{|}~
    product_name = "".join([char for char in product_name if char not in string.punctuation])
...
general.add_argument("--output", default="/workspace/datasets/categories/output_orig.fasttext", help="the file to output to")
            ...
            if filename.endswith(".xml"):
                ...
                for child in root:
                    ...
                    if (child.find('name') is not None and child.find('name').text is not None and
                        ...
                        output.write("__label__%s %s\n" % (cat, transform_name(name, stemmer=False)))

# filter my min_products
print(f"Value of min_products: {len(df)}")
df = pd.read_csv('/workspace/datasets/categories/output_orig.fasttext', names=['cat_name'])
...
df.to_csv("/workspace/datasets/categories/output_filtered.fasttext", header=False, index=False)